...
/*ALUNOS:
    ...
    IMPLEMENTAÇÃO DAS FUNÇÕES PARA O TOAST (INSERT/SELECT)
...
/*DEIVIDI LUIZ - WILLIAM TOMAZINI

Função para fazer a página toast do varchar, onde vai ser criado um novo arquivo quando tiver mais de 2k,
e quando essa página toast tiver 8k ocupados, vai ser criado uma nova págia toast em sequencia, quando elas
forem relacionadas, serão separadas com o hífen '-', onde o primeiro numero é relacionado a página e o segundo 
sempre que precisar um novo toast. Coloca o caractere & no final para saber o final do arquivo, quando precisar 
percorre-lo, nessa página é salvo o tamanho da memoria livre e também o tamanho de memoria usada. Caractere $ colocado
no começo do varchar para saber onde começa.

 */

int toastVarchar(char *token, char tableName[500], int numPage) { 
    char pageName[600];
    char endVarchar = '$', caractereChar2 = '&', charInFile = ' ';
    int numPage2 = 0;

    snprintf(pageName, sizeof(pageName), "%s/toastv%d.dat", tableName, numPage); 
    FILE *page2 = fopen(pageName, "rb+"); // abre a pagina como modo de leitura e escrita binaria
    
    toast tooast;

    if(page2 == NULL) { //caso tenha ocorrido algum erro na criação da pagina
        page2 = fopen(pageName, "w"); //cria o arquivo da pagina no modo de escrita binária

        tooast.memFree = 8190;
        tooast.memUsed = 0;

        fseek(page2, 0, SEEK_SET);
        fwrite(&tooast.memFree, sizeof(int), 1, page2);  // quantidade de memória disponível
        fwrite(&tooast.memUsed, sizeof(int), 1, page2);  // quantidade de memória usada 

        if(tooast.memUsed == 0){
            fwrite(&caractereChar2, 1, 1, page2); //&
        }

        fclose(page2);
        page2 = fopen(pageName, "rb+");
    }else{
        fseek(page2, 0, SEEK_SET);
        fread(&tooast.memFree, sizeof(int), 1, page2);  // quantidade de memória disponível
        fread(&tooast.memUsed, sizeof(int), 1, page2);  // quantidade de memória usada 

        if(tooast.memUsed > 8190){
            do{
                numPage2++;
                snprintf(pageName, sizeof(pageName), "%s/toastv%d-%d.dat", tableName, numPage, numPage2); 
                page2 = fopen(pageName, "rb+"); // abre a pagina como modo de leitura e escrita binaria
                ...
                fread(&tooast.memFree, sizeof(int), 1, page2);  // quantidade de memória disponível
                fread(&tooast.memUsed, sizeof(int), 1, page2);  // quantidade de memória usada                 
            }while(tooast.memUsed > 8190);

            if(page2==NULL){
                snprintf(pageName, sizeof(pageName), "%s/toastv%d-%d.dat", tableName, numPage, numPage2);
                page2 = fopen(pageName, "w");
    ...
    fread(&charInFile, 1, 1, page2);

    if(charInFile == '&'){
        fseek(page2, -1, SEEK_CUR);
    }else{
        do{
            fread(&charInFile, 1, 1, page2);
        }while(charInFile != '&');
    ...
    fwrite(&endVarchar, 1, 1, page2);
    fwrite(token, strlen(token), 1, page2);
    fwrite(&caractereChar2, 1, 1, page2);
    fclose(page2);

    printf("New toast inserted\n");
...
Função para fazer a página toast do char, onde vai ser criado um novo arquivo quando tiver mais de 2k,
e quando essa página toast tiver 8k ocupados, vai ser criado uma nova págia toast em sequencia, quando elas
forem relacionadas, serão separadas com o hífen '-', onde o primeiro numero é relacionado a página e o segundo 
sempre que precisar um novo toast. Coloca o caractere & no final para saber o final do arquivo, quando precisar 
percorre-lo, nessa página é salvo o tamanho da memoria livre e também o tamanho de memoria usada. Caractere # colocado
no começo do char para saber onde começa. Caso o char tiver tamanho 2k ou maior e for inserido apenas 1 caractere, ele
vai preencher todos os outros epaços com caractere $.

 */

int toastChar(char *token, char tableName[500], int posAtributo) { 
    char pageName[600];
    char caractereChar = '$', charInFile, caractereChar2 = '&', caractereChar3 = '#';
    int numPage = 1, numPage2 = 0;

    snprintf(pageName, sizeof(pageName), "%s/toastc%d.dat", tableName, numPage); 
    FILE *page2 = fopen(pageName, "rb+"); // abre a pagina como modo de leitura e escrita binaria
    ...
    if(page2!=NULL){
        if(tooast.memFree > 0){
            ...
            fwrite(&tooast.memFree, sizeof(int), 1, page2);  // quantidade de memória disponível
            fwrite(&tooast.memUsed, sizeof(int), 1, page2);  // quantidade de memória usada               
        }else{  //CASO O TOAST NÃO TIVER MAIS ESPAÇO
            fseek(page2, 0, SEEK_SET);
            fread(&tooast.memFree, sizeof(int), 1, page2);  // quantidade de memória disponível
            fread(&tooast.memUsed, sizeof(int), 1, page2);  // quantidade de memória usada 
            ...
            if(tooast.memUsed > 8190){
                do{
                    numPage2++;
                    snprintf(pageName, sizeof(pageName), "%s/toastc%d-%d.dat", tableName, numPage, numPage2); 
                    page2 = fopen(pageName, "rb+"); // abre a pagina como modo de leitura e escrita binaria
    ...
    fwrite(&caractereChar3, 1, 1, page2);
    fwrite(token, strlen(token), 1, page2);

    if(strlen(token) < 2047){
        for(int i = strlen(token); i<2048; i++){
            fwrite(&caractereChar, 1, 1, page2);
        }
    }
...
void buildHeader(char *sql, char *tableName, int qtdPages) {
    ...
    char sqlCopy[4500], pageName[600];  
    char *token;
    ...
    while(token != NULL) { // enquanto existir tokens
        ...
        if(strcmp(token, "char") == 0) { // compara se o token é um char
            ...
            Quando o tamanho do char informado for maior ou igual 2k, já vai criar automaticamente o arquivo toast
            para o char, definido o tipo para 'T' para facilitar nas inserções e na impressão.
            ...
            if(fieldSize >= 2048){
                ...
                do{
                    snprintf(pageName, sizeof(pageName), "%s/toastc%d.dat", tableName, numPage); 
                    toast = fopen(pageName, "r"); // abre a pagina como modo de leitura e escrita binaria
                    numPage++;
                }while(toast != NULL);
                numPage--;
                ...
                if(toast == NULL){
                    snprintf(pageName, sizeof(pageName), "%s/toastc%d.dat", tableName, numPage); 
                    toast = fopen(pageName, "wb"); // abre a pagina como modo de leitura e escrita binaria
                }
                
                fwrite(&tooast.memFree, sizeof(int), 1, toast);  // quantidade de memória disponível
                fwrite(&tooast.memUsed, sizeof(int), 1, toast);  // quantidade de memória usada

                fclose(toast);

                fieldType = 'T';
                fieldSize = 3;
                printf("New toast created\n");
            }
...
void insertInto(char *sql, int numPage) { 
    char sqlCopy[4500], *token, tableName[500], pageName[600], attrSql[4500], attrSqlCopy[4500];
    char endVarchar = '$', special = ' ', endChar = '\0';
    char endVarcharToast = '#';
    char endCharToast = '&';
    int insertSize = 0, qtdFields, nextItem, intVar, nextPage, qtdEndChar = 0, bkpInsertSize = 0;
    int i = 0;
    ...
    for(int i = 0; i < qtdFields; i++) {        
        if(attributes[i].type == 'C') {
            ...
            bkpInsertSize = insertSize; //CRIANDO BKP PARA O TAMNHO DO INSERT
            insertSize += strlen(token) + 1; // + 1 for special char of varchar '$'

            if(insertSize >= 2048){ //2048(2kb) 
                insertSize = bkpInsertSize+3; //RESETA O TAMANHO DO INSERT, APENAS ACRESCENTANDO 3 POIS O VARCHAR VAI ESTAR
                                              //NO ARQVUIO TOAST
            }            
        }else if(attributes[i].type == 'T'){
            insertSize += attributes[i].size;
        }
        token = strtok(NULL, ",");
    }
    ...
    if(head.memFree > insertSize) {
        ...
        for(int i = 0; i < qtdFields; i++) {
            ...
            // int
            } else if(attributes[i].type == 'T'){ //TIPO T É O CHAR, COMO ELE JÁ TEM UM CRIADO INICIALMENTE, SEMPRE VAI
                //printf("%d\n", i);                //ADICIONAR NO TOAST, ATÉ TER 8K, QUANDO TIVER, CRIA OUTRO RELACIOADO.
                int numeroToastChar = toastChar(token, tableName, i+1);
                fwrite(&endCharToast, 1, 1, page); 
                fwrite(&numeroToastChar, 1, 1, page);
                fwrite(&endCharToast, 1, 1, page);
            }else if(attributes[i].type == 'I') {
                intVar = atoi(token);
            ...
            // varchar
                ...
                if(strlen(token) < 2048){ //SE NÃO FOR ARQUIVO TOAST APENAS ESCREVE
                    fwrite(token, strlen(token), 1, page);
                    fwrite(&endVarchar, 1, 1, page);
                }else if(strlen(token) >= 2048){ //SE FOR CHAMA A FUNÇÃO TOAST
                    //fclose(page);
                    int numeroToast = toastVarchar(token, tableName, 1);
                    fwrite(&endVarcharToast, 1, 1, page); 
                    fwrite(&numeroToast, 1, 1, page);
                    fwrite(&endVarcharToast, 1, 1, page);                    
                }
            }
            token = strtok(NULL, ",");
...
void selectFrom(char *sql, int numPage) {
    char tableName[50], sqlCopy[4500], pageName[600], *token, charInFile, special = ' ';
    int i = 0, qtdFields = 0, moveItem = 0, intInFile, stopChar;
    ...
    for(int i = 0; i < head.qtdItems; i++) { // laço para percorrer os itens
        ...
        for(int j = 0; j < qtdFields; j++) {  
            ...
            // se o atributo for varchar
                ...
                do {
                    fread(&charInFile, 1, 1, page);
                    if(charInFile != '$' && charInFile != '#') //# CARACTERE PARA SABER QUE É TOAST
                        printf("%c", charInFile);
                } while(charInFile != '$' && charInFile != '#');

                //QUANDO ENCONTRAR O CARACTERE PERCORRE UMA POSIÇÃO PARA VER SE O PROXIMO É #
                if(charInFile == '$')
                    fread(&charInFile, 1, 1, page);
                
                if(charInFile == '#'){ //USADO PARA LER O TOAST
                    int numeroToast = 0;

                    fread(&charInFile, 1, 1, page);
                    char *bkp = &charInFile; //USADO PARA PEGAR O VALOR QUE ESTA ENTRE # # PARA ABRIR O ARQUIVO

                    numeroToast = *bkp;  
                    snprintf(pageName, sizeof(pageName), "%s/toastv%d.dat", tableName, numeroToast); // define o nome da pagina da tabela
                    FILE *pageToast = fopen(pageName, "r"); // abre a pagina da tabela em modo de leitura e escrita binaria
                    ...
                    fseek(pageToast, 0, SEEK_SET);
                    fread(&tooast2.memFree, sizeof(int), 1, pageToast);  // quantidade de memória disponível
                    fread(&tooast2.memUsed, sizeof(int), 1, pageToast);  // quantidade de memória usada                 

                    /* DEIVIDI LUIZ - WILLIAM TOMAZINI
                    VAI SER PECORRIDO ATÉ bI FOR DIFERENTE DE i, POIS NESSE CASO O I VAI PERCORRER ATÉ FOR MAIOR QUE A 
                    QUANTIDADE DE ITENS, E TODA VEZ QUE ENCONTRAR O CARACTERE $ VAI SENDO SOMADO ATÉ QUE BI SEJA IGUAL 
                    A I+1, QUANDO ENCONTRADO UTILIZADO O BREAK PARA SAIR DOS LAÇOS, E COMEÇAR IMPRIMIR DE ONDE PAROU, CASO NÃO 
                    ENCONTRE VAI PERCORRER ATÉ ENCONTRAR O CARACTERE DO FINAL, &, APOS VAI VER SE EXISTE UM ARQUIVO TOAST
                    RELACIONADO, SEPARADO POR '-'.               
                    */
                    do{        
                        do{
                            fread(&charInFile, 1, 1, pageToast);
                        ...
                        if(charInFile == '&' && bI!=i+1){
                            fclose(pageToast);
                            snprintf(pageName, sizeof(pageName), "%s/toastv%d-%d.dat", tableName, numeroToast, numeroToast2); // define o nome da pagina da tabela
                            pageToast = fopen(pageName, "r"); // abre a pagina da tabela em modo de leitura e escrita binaria
                            numeroToast2++;
                    ...
                    /*DEIVIDI LUIZ - WILLIAM TOMAZINI
                    SOMENTE VAI PERCORRER ATÉ O FINAL DA PAGINA QUE ESTA, E QUANDO FOR DIFERENTE DOS SIMBOLOS VAI 
                    FAZER A IMPRESSÃO.
        
                    */
                    if(pageToast != NULL){
                        if(charInFile != '$')
                            fseek(pageToast, -1, SEEK_CUR);

                        do { 
                            fread(&charInFile, 1, 1, pageToast);

                            if(charInFile != '&' && charInFile != '#' && charInFile != '$') //# CARACTERE PARA SABER QUE É TOAST
                                printf("%c", charInFile);
                ...
                /*FUNCIONA DA MESMA FORMA QUE PARA OS VARCHAR, PORÉM TROCANDO OS SIMBOLOS PARA SABER DIFERENCIAR*/
                if(charInFile == '&'){
                    charInFile = ' ';
                    ...
                    snprintf(pageName, sizeof(pageName), "%s/toastc%d.dat", tableName, numeroToast); // define o nome da pagina da tabela
                    FILE *pageToast = fopen(pageName, "rb+"); // abre a pagina da tabela em modo de leitura e escrita binaria
                    ...
                    do{        
                        ...
                        if(charInFile == '&'){
                            fclose(pageToast);
                            snprintf(pageName, sizeof(pageName), "%s/toastc%d-%d.dat", tableName, numeroToast, numeroToast2); // define o nome da pagina da tabela
                            pageToast = fopen(pageName, "r"); // abre a pagina da tabela em modo de leitura e escrita binaria
                            numeroToast2++;
...
void deleteFrom(char *sql, int numPage) {
    char tableName[50], sqlCopy[4500], pageName[600], *token, charInFile, attributeName[20], special = ' ';
...
int main() {
    char sql[4500], operation[10];

    system("clear");
    do {
        printf(">> ");
        fgets(sql, 4500, stdin);
        getOp(sql, operation);
